...
# 镜像变量
ARG DOCKER_IMAGE=iflyelf/dnsproxy
...
RUN set -eux && \
    # 映射架构到 dnsproxy 包名
    case "${TARGETARCH}" in \
        amd64)   DNSPROXY_ARCH=linux-amd64   ;; \
        arm64)   DNSPROXY_ARCH=linux-arm64   ;; \
        arm)     DNSPROXY_ARCH=linux-armv7   ;; \
        *)       echo "不支持的架构: ${TARGETARCH}" && exit 1 ;; \
    esac && \
    echo "下载 dnsproxy for ${DNSPROXY_ARCH}" && \
    DNSPROXY_URL=$(curl -s https://api.github.com/repos/AdguardTeam/dnsproxy/releases/latest | jq -r ".assets[] | select(.name | contains(\"${DNSPROXY_ARCH}\")) | .browser_download_url") && \
    echo "下载地址: ${DNSPROXY_URL}" && \
    wget --no-check-certificate -O /tmp/dnsproxy.tar.gz "${DNSPROXY_URL}" && \
    cd /tmp && tar xzf dnsproxy.tar.gz && \
    # 查找解压后的 dnsproxy 二进制文件(可能在子目录中)
    find /tmp -name dnsproxy -type f -executable -exec mv {} /usr/bin/dnsproxy \; && \
...
# 暴露端口
EXPOSE 5353/udp 5353/tcp 853/tcp 853/udp 813/tcp 813/udp
